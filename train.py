...
def parse_args():
    ...
    parser.add_argument('--save', dest='resume', default='net_model/_9129_food/weights_5_3049_0.9788_0.9991.pth', type=str)
    parser.add_argument('--backbone',
                        dest='backbone',
                        default='vit_large_patch16_384',
                        type=str)
    parser.add_argument('--auto_resume',
                        dest='auto_resume',
                        action='store_true')
    parser.add_argument('--epoch', dest='epoch', default=3600, type=int)
    parser.add_argument('--tb', dest='train_batch', default=104, type=int)
    parser.add_argument('--vb', dest='val_batch', default=208, type=int)
    ...
    parser.add_argument('--lr', dest='base_lr', default=0.0005, type=float)
    parser.add_argument('--lr_step', dest='decay_step', default=20, type=int)
    parser.add_argument('--cls_lr_ratio',
                        dest='cls_lr_ratio',
                        default=10.0,
                        type=float)
...
def auto_load_resume(load_dir):
    ...
    acc_list = [
        x[:-4].split('_')[-1] if x[:7] == 'weights' else 0 for x in weight_list
    ]
...
if __name__ == '__main__':
    ...
    if args.resume:
        ...
        pretrained_dict = torch.load(resume, map_location='cpu')
    ...
    filename = '%s_%d%d%d_%s' % (args.discribe, time.month, time.day,
                                 time.hour, Config.dataset)
    ...
    if Config.use_backbone:
        ignored_params = list(map(id, module.classifier.parameters()))
    else:
        ignored_params1 = list(map(id, module.classifier.parameters()))
        ignored_params2 = list(map(id, module.classifier_swap.parameters()))
        ignored_params3 = list(map(id, module.Convmask.parameters()))
    ...
    if Config.use_backbone:
        ...
        {
            'params': module.classifier.parameters(),
            'lr': base_lr
        }],
                              lr=base_lr,
                              momentum=0.9)
        ...
        optimizer = optim.SGD([
            ...
            {
                'params': module.classifier_swap.parameters(),
                'lr': lr_ratio * base_lr
            },
            {
                'params': module.Convmask.parameters(),
                'lr': lr_ratio * base_lr
            },
        ],
                              lr=base_lr,
